          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map((user, index) => (
              <tr key={user.id || index} className="hover:bg-gray-50 block md:table-row">
                <td className="px-4 md:px-6 py-2 md:py-4 whitespace-nowrap text-sm font-medium text-gray-900 flex md:table-cell justify-between"><span className="md:hidden text-gray-500">ID</span><span>{user.id}</span></td>
                <td className="px-4 md:px-6 py-2 md:py-4 whitespace-nowrap text-sm text-gray-500 flex md:table-cell justify-between"><span className="md:hidden text-gray-500">Хэрэглэгчийн нэр</span><span>{user.name}</span></td>
                <td className="px-4 md:px-6 py-2 md:py-4 whitespace-nowrap text-sm text-gray-500 flex md:table-cell justify-between"><span className="md:hidden text-gray-500">И-мэйл</span><span>{user.email}</span></td>
                <td className="px-4 md:px-6 py-2 md:py-4 whitespace-nowrap text-sm text-gray-500 flex md:table-cell justify-between"><span className="md:hidden text-gray-500">Үүрэг</span><span>{user.role}</span></td>
                <td className="px-4 md:px-6 py-2 md:py-4 whitespace-nowrap text-sm flex md:table-cell justify-between">
                  ...
                  <span>
                    <button onClick={() => onEdit(index)} className="text-indigo-600 hover:text-indigo-900 mr-3 font-medium transition-colors">Засах</button>
                    <button onClick={() => onDelete(index)} className="text-red-600 hover:text-red-900 font-medium transition-colors">Устгах</button>
                  </span>
                </td>
              </tr>
            ))}
...
// Reused from other admin components, ensuring consistent UI for dialogs
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm transform transition-all scale-100 opacity-100">
        ...
        <div className="flex justify-end space-x-3 mt-4">
          {type === "confirm" && (
            <button onClick={onCancel} className="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 transition-colors">Үгүй</button>
          )}
          <button onClick={type === "alert" ? onClose : onConfirm} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">{type === "alert" ? "Хаах" : "Тийм"}</button>
        </div>
      </div>
    </div>
  );
...
const UserAdmin = () => {
  ...
  const showAlert = useCallback((message, title = "Мэдээлэл") => {
    setModal({ isOpen: true, title, message, type: "alert", onClose: () => setModal((prev) => ({ ...prev, isOpen: false })) });
  }, []);
  ...
  const showConfirm = useCallback((message, onConfirmCallback, title = "Баталгаажуулах") => {
    setModal({
      ...
      message,
      type: "confirm",
      onConfirm: () => { onConfirmCallback(); setModal((prev) => ({ ...prev, isOpen: false })); },
  ...
  const handleChange = (e) => {
    ...
    setCurrentUser((prev) => ({ ...prev, [name]: name === "id" ? (value === "" ? "" : Number(value)) : value }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingIndex !== null) {
        await apiClient.users.update(users[editingIndex].id, { name: currentUser.name, email: currentUser.email, role: currentUser.role, password: currentUser.password || undefined });
      } else {
        await apiClient.users.create({ name: currentUser.name, email: currentUser.email, role: currentUser.role, password: currentUser.password });
      }
  ...
  const handleEdit = (index) => {
    ...
    setCurrentUser({ id: userToEdit.id ?? "", name: userToEdit.name ?? "", email: userToEdit.email ?? "", role: userToEdit.role ?? "user", password: "" });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 font-sans antialiased">
      ...
      <button onClick={() => { setFormVisible(!formVisible); setEditingIndex(null); setCurrentUser({ id: "", name: "", email: "", role: "user", password: "" }); }} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md mb-6 shadow-md transition-colors">
        {formVisible ? "Хаах" : "Хэрэглэгч нэмэх"}
      </button>

      {formVisible && (
        <div className="bg-white shadow-md rounded-lg p-6 space-y-4 mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">{editingIndex !== null ? "Хэрэглэгч засах" : "Шинэ хэрэглэгч нэмэх"}</h2>
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Хэрэглэгчийн нэр:</label>
            <input type="text" id="name" name="name" value={currentUser.name} onChange={handleChange} className="input-field" />
          </div>
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">И-мэйл:</label>
            <input type="email" id="email" name="email" value={currentUser.email} onChange={handleChange} className="input-field" />
          </div>
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-gray-700 mb-1">Нууц үг:</label>
            <input type="password" id="password" name="password" value={currentUser.password} onChange={handleChange} className="input-field" placeholder={editingIndex !== null ? "Шинэ нууц үг оруулахгүй бол өөрчлөгдөхгүй" : ""} />
          </div>
          <div>
            <label htmlFor="role" className="block text-sm font-medium text-gray-700 mb-1">Үүрэг:</label>
            <select id="role" name="role" value={currentUser.role} onChange={handleChange} className="input-field">
